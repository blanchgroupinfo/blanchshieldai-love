...
import { 
  Calendar, Sun, Moon, Star, Book, Clock, ChevronLeft, ChevronRight, 
  Sparkles, Sunrise, AlertCircle, Printer, MapPin, Plus, Bell,
  Search, Navigation, X, Sunset, Edit, Trash2
} from "lucide-react";
                ...
                <div className="grid grid-cols-2 gap-4 py-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setCoordInput(prev => ({ ...prev, lon: e.target.value }))}
                    />
                  </div>
                </div>
                <DialogFooter>
                ...
                <CardHeader>
                  ...
                  <div className="text-center mb-4 p-3 rounded-lg bg-amber-500/5 border border-amber-500/20">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Anchor: March 17, 2013 = Day 1, Month 1 • Before the Equilux
                    </p>
                  </div>
                      ...
                      <TableHeader>
                        <TableRow className="bg-card/80">
                          {hebrewDayNames.map((dayInfo, index) => (
                            <TableHead 
                              key={dayInfo.day} 
                              className={`text-center min-w-[110px] ${index === 6 ? 'bg-amber-500/10 text-amber-400' : ''}`}
                            >
                          ...
                          <TableRow key={weekIndex}>
                            {week.map((day, dayIndex) => {
                              ...
                              return (
                                <TableCell 
                                  key={dayIndex}
                                  className={`border border-border/30 p-1.5 align-top cursor-pointer transition-colors min-h-[90px]
                                ...
                                >
                                  <div className="flex flex-col gap-0.5">
                                    <div className="flex items-center justify-between">
                                      ...
                                      {day === 1 && (
                                        <Badge variant="outline" className="text-[8px] px-1 py-0 bg-blue-500/20 text-blue-300 border-blue-500/30">
                                          New
                                        </Badge>
                                      )}
                                    </div>
                                    ...
                                    {dayFeasts.map((feast, i) => (
                                      <Badge 
                                        key={i} 
                                        variant="outline" 
                                        className={`text-[8px] px-1 py-0 ${getFeastBadgeColor(feast.type)}`}
                                      >
                                    ...
                                    {dayEvents.map((event, i) => (
                                      <Badge 
                                        key={i} 
                                        variant="outline" 
                                        className="text-[8px] px-1 py-0 bg-primary/20 text-primary border-primary/30"
                                      >
                                    ...
                                    {dayIsSabbath && (
                                      <span className="text-[9px] text-amber-400 font-medium">SHABBAT</span>
                                    )}
                                  </div>
                                </TableCell>
                              );
                  ...
                  {selectedDay && (
                    ...
                    >
                      ...
                      <p className="text-sm text-muted-foreground mb-2">
                        ...
                        {isSabbath(selectedDay) && (
                          <Badge className="ml-2 bg-amber-500/20 text-amber-300">SHABBAT</Badge>
                        )}
                      </p>
                      ...
                      {getFeastsForDay(monthData.monthNumber, selectedDay).map((feast, i) => (
                        <div key={i} className="mt-2 p-3 rounded bg-primary/5 border border-primary/20">
                          ...
                          {feast.scriptures && feast.scriptures.length > 0 && (
                      ...
                      {/* User Events */}
                      {getEventsForDay(monthData.monthNumber, selectedDay).map((event) => (
                        <div key={event.id} className="mt-2 p-3 rounded bg-primary/5 border border-primary/20">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                            ...
                            <Button 
                              variant="ghost" 
                              size="sm" 
                              className="text-destructive hover:text-destructive no-print"
                      ...
                      {!user && (
                        <p className="text-sm text-muted-foreground mt-3 no-print">
                  ...
                  <div className="mt-4 p-4 rounded-lg bg-muted/30 border border-border/30">
                    ...
                    <ul className="text-sm text-muted-foreground space-y-1">
                      ...
                      {monthFeasts.filter(f => f.type !== 'new-month').map((feast, i) => (
                        <li key={i}>• {feast.name} ({feast.hebrewName}) - Day {feast.day}{feast.endDay !== feast.day ? `-${feast.endDay}` : ''}</li>
                      ))}
                    </ul>
                  </div>
                </CardContent>
                ...
                <CardContent>
                  <div className="grid md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {calendarMonths.map((month, index) => {
                      const monthStart = getGregorianDate(currentYear, month.monthNumber, 1);
                      const monthEnd = getGregorianDate(currentYear, month.monthNumber, 30);
                      const monthHolyDays = feasts.filter(f => f.month === month.monthNumber && f.type !== 'new-month');
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2 mb-2">
                            <span className="w-10 h-10 rounded-full bg-amber-500/20 flex items-center justify-center text-amber-400 font-bold">
                              {month.monthNumber}
                            </span>
                            <div>
                          ...
                          <p className="text-xs text-muted-foreground mb-2">
                            {formatGregorianDate(monthStart)} - {formatGregorianDate(monthEnd)}
                          </p>
                          
                          <div className="space-y-1">
                            {monthHolyDays.slice(0, 3).map((feast, i) => (
                              <Badge 
                                key={i} 
                                variant="outline" 
                                className={`text-[9px] block truncate ${getFeastBadgeColor(feast.type)}`}
                              >
                                Day {feast.day}: {feast.name}
                              </Badge>
                            ))}
                            {monthHolyDays.length > 3 && (
                              <span className="text-[9px] text-muted-foreground">+{monthHolyDays.length - 3} more</span>
                            )}
                          </div>
                        </motion.div>
                      );
                  ...
                  <CardContent className="space-y-3">
                    {feasts.filter(f => f.month <= 3 && f.type !== 'new-month').map((feast, index) => (
                      ...
                      >
                        ...
                        {feast.noWork && (
                          <Badge variant="outline" className="mt-2 bg-red-500/10 text-red-300 text-[10px]">No Work</Badge>
                        )}
                      </motion.div>
                    ))}
                  </CardContent>
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between">
                              <Select
                                ...
                                onValueChange={(value) => setReminder(feast.name, parseInt(value), true)}
                              >
                              ...
                              <Switch
                                ...
                                onCheckedChange={(checked) => 
                                  setReminder(feast.name, reminder?.remind_days_before || 1, checked)
                                }
                              />
                            </div>
                          </div>
