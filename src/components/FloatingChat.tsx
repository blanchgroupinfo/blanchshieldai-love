...
const FloatingChat = () => {
  ...
  return <>
      ...
      <AnimatePresence>
        ...
        scale: 0,
        opacity: 0
      }} onClick={() => setIsOpen(true)} className="fixed bottom-6 right-6 z-50 p-4 rounded-full bg-primary text-primary-foreground shadow-2xl hover:shadow-primary/50 transition-all duration-300" whileHover={{
        scale: 1.1
      }} whileTap={{
        scale: 0.9
      }}>
      ...
      <AnimatePresence>
        ...
        scale: 0.95
            ...
            y: 0
                  ...
                  <div className={`shrink-0 w-7 h-7 rounded-lg flex items-center justify-center ${message.role === "user" ? "bg-primary/20" : "bg-shield-accent/20"}`}>
                    {message.role === "user" ? <User className="w-3 h-3 text-primary" /> : <Bot className="w-3 h-3 text-shield-accent" />}
                  </div>
                  <div className={`max-w-[80%] p-3 rounded-xl text-xs ${message.role === "user" ? "bg-primary text-primary-foreground rounded-tr-md" : "bg-card border border-border/50 text-foreground rounded-tl-md"}`}>
            ...
            <div className="p-3 border-t border-border/50 bg-card/50">
              <div className="flex items-end gap-2">
                ...
                <Button variant="ghost" size="icon" onClick={handleFileUpload} className="shrink-0 text-muted-foreground hover:text-primary h-8 w-8">
                  <Paperclip className="w-4 h-4" />
                </Button>
                <Button variant="ghost" size="icon" onClick={toggleRecording} className={`shrink-0 h-8 w-8 ${isRecording ? "text-red-500 animate-pulse" : "text-muted-foreground hover:text-primary"}`}>
                  {isRecording ? <MicOff className="w-4 h-4" /> : <Mic className="w-4 h-4" />}
                </Button>
                <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="Ask S.H.I.E.L.D. AI..." className="flex-1 bg-background border border-border/50 rounded-xl px-3 py-2 text-xs resize-none focus:outline-none focus:ring-2 focus:ring-primary/50 min-h-[36px] max-h-[80px]" rows={1} />
